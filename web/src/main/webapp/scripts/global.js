...
function checkRequired(elementId, elementDescription) {
    if (document.getElementById(elementId)) {
        element = document.getElementById(elementId);
    	if (element.value == '') {
    		window.alert(elementDescription + ' is a required field.');
    		return false;
    	}
    }
    return true;
...
window.onload = function() {
   ...
   /*
    * Display calendar when a button/calendar icon is clicked
    */
   function displayCalendar(button){
      var c = CalendarPopup();
      if(document.getElementById(button.name)){
         //set target input, button's name should be the same as input's id
         window.CP_targetInput = document.getElementById(button.name);
         c.showCalendar(button.id);
      }
   }
